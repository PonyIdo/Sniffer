...
unsigned uint16_t getPacket_destination_port(SK_Buff *skbuff){
    unsigned char *headptr = skbuff->head;
    uint16_t transport_offset = skbuff->transport_header;

    unsigned char *transport_headerptr = headptr + transport_offset;
    unsigned int destination_port_offset = 16;
    unsigned char *destination_portptr = transport_headerptr + destination_port_offset;
    unsigned uint16 destination_port = *destination_portptr;
    return destination_port;
}

unsigned uint16_t getPacket_source_port(SK_Buff *skbuff){
    unsigned char *headptr = skbuff->head;
    uint16_t transport_offset = skbuff->transport_header;
    ...
    unsigned uint16 source_port = *source_portptr;
    return source_port;
}